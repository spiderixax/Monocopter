...
int currentServoAngle = default_angle_1;
float flap_constant = 0.06;
...
void loop() {
  ...
  if (cycleCounter == cycleDelay) {
    ...
    voltageReading = analogRead(A7)*0.00709;
    int voltageInt = int(voltageReading);
    for (int i = 0; i < voltageReading; i++) {
  ...
  if (flight_magnitude > 40) {
    float sineValue = sin(0.01745329 * (current_bearing - flight_bearing) + 4.712389);
    if (sineValue > 0.2) {
      servo_angle_1 = neutral_angle_1 - (flap_constant * flight_magnitude);
    }
    else if (sineValue < -0.2) {
      servo_angle_1 = neutral_angle_1 + (flap_constant * flight_magnitude);
    }
  ...
  //int servoUpdateDifference = ((int(currentServoAngle - servo_angle_1)) ^ 2);
  int servoUpdateDifference = currentServoAngle - servo_angle_1;

  if (servoUpdateDifference > 5 or servoUpdateDifference < -5) {
    Servo1.write(servo_angle_1);
    currentServoAngle = servo_angle_1;
  }

  else {}
  
  //Serial.println(servoUpdateDifference);
