...
void loop() {
  ...
  if (flight_magnitude > 40) {
    servo_angle_flap = default_angle_flap - (flap_constant * flight_magnitude * 57.29578 * sin(0.01745329 * (current_bearing - flight_bearing) + 4.712389 - 1.57080));
  }
  ...
  if (wingUpdateDifference > 1 or wingUpdateDifference < -1) {
    ServoWing.write(servo_angle_wing);
    currentWingAngle = servo_angle_wing;
  }
  ...
  if (flapUpdateDifference > 1 or flapUpdateDifference < -1) {
