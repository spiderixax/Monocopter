...
void loop() {
  ...
  if (cycleCounter == cycleDelay) {                                           //  A section of code executed every (cycleDelay)th cycle, 
    ...
    if (voltageReading > 5.00 and voltageReading < 7.45) {     //  want it to check (I use Channel 3 for this)
  ...
  if (flight_magnitude > 60) {
    //servo_angle_wing = default_angle_wing - (flap_constant * flight_magnitude * sin(0.01745329 * (current_bearing - flight_bearing) + 3.14159));
    motor_power = z_vec - (flap_constant * flight_magnitude * sin(0.01745329 * (current_bearing - flight_bearing) + 3.14159));
  }

  else if (flight_magnitude <= 60) {
    //servo_angle_wing = default_angle_wing;
    motor_power = z_vec;
  }

  //motor.write(servo_angle_wing);
