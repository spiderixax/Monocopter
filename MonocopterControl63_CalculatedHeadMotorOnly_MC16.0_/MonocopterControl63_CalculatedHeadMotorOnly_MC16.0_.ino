...
int default_angle_wing = 100;
int servo_angle_wing = default_angle_wing;

float flap_constant = 0.5;
...
void loop() {
  ...
  heading = correctedHeading(heading);

  if (heading >= 250 and heading <=360 and rotation_reset == true) {
  ...
  else if (heading >= 20 and heading <= 230 and rotation_reset == false) {
    rotation_reset = true;
  ...
  if (current_bearing >= 280 and current_bearing < 320) {
    digitalWrite(redLedMCCU, HIGH);
  }
  ...
  if (current_bearing > 100 and current_bearing < 140) {
    digitalWrite(blueLedMCCU, HIGH);
  }
...
/*
  ...
  if (flight_magnitude > 40) {
    //motor_power = z + (flap_constant * flight_magnitude * sin(0.01745329 * (current_bearing - flight_bearing) - 1.5708));
    motor_power = z + (flap_constant * flight_magnitude * sin(0.01745329 * (current_bearing - flight_bearing) ));
    //motor_power = z + (flap_constant * flight_magnitude * sin(0.01745329 * (heading - flight_bearing) ));
  }
