#define coil_1 7
#define coil_2 8
#define light_gate_1 A0
#define light_gate_2 A6
...
void loop() {
  ...
  if (z_vec < -60 and fired == false) {
    //Serial.println(sin(0.01745329 * (current_bearing - flight_bearing) + 4.712389));

    if (sin(0.01745329 * (current_bearing - flight_bearing) + 4.712389 + 0.523599) < -0.70){
      fire_coil_gun();
      //Serial.println("fire");
      fired = true;
    }
...
void fire_coil_gun() {
  ...
  firing_time = millis();
    //Serial.println("Coil 1 ON");
    
  while (analogRead(light_gate_1) > 140 and (millis() - firing_time) < 50) {
    //Serial.println(analogRead(light_gate_1));
    delay(1);
  }

  Serial.println(millis() - firing_time);

  digitalWrite(coil_1, LOW);
  digitalWrite(coil_2, HIGH);
  firing_time = millis();
  //Serial.println("Coil 1 OFF, Coil 2 ON");
    
  while (analogRead(light_gate_2) > 150 and (millis() - firing_time) < 30) {
    //Serial.println(analogRead(light_gate_1));
    delay(1);
  }
