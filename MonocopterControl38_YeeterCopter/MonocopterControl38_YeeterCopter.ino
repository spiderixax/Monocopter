...
float default_angle_yeeter = 90;
float servo_angle_yeeter = default_angle_yeeter;
int yeeter_constant = 70;
...
void setup() {
  ...
  ServoYeeter.attach(yeeterPin);
  ServoYeeter.write(default_angle_yeeter);
...
*/
  ...
  if (z_vec < -60) {
    servo_angle_yeeter = default_angle_yeeter + (yeeter_constant * sin(0.01745329 * (current_bearing - flight_bearing) -1));
    if (servo_angle_yeeter > default_angle_yeeter) {
      ServoYeeter.write(servo_angle_yeeter);
    }
    else {
      ServoYeeter.write(default_angle_yeeter);
    }
