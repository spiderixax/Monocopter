...
QMC5883L compass;
//Servo motor;
ESC motor (motorPin, 1000, 2000, 500);
...
void loop() {
  ...
  if (cycleCounter == cycleDelay) {                                           //  A section of code executed every (cycleDelay)th cycle, 
    voltageReading = analogRead(A7)*0.01565;                                  //  here it checks if the battery voltage (measured by a
    ...
    else if (voltageReading > 10 and voltageReading < 11.4 and t < 1200) {
      digitalWrite(buzzerPin, HIGH);
    }
    else {
  ...
  */
  ...
  //z = map(z, 1100, 1750, 1000, 2000);

  //safety = map(t, 1450, 1570, 0, 255);
  ...
  if (flight_magnitude > 40) {
    ...
    motor_power = z - (flap_constant * flight_magnitude * sin(0.01745329 * (current_bearing - flight_bearing) + 3.14159 + 1.5708));
  }
  ...
  motor_power = constrain(motor_power, 1000, 2000);
  motor.speed(motor_power);
