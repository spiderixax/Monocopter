...
void loop() {
  ...
  if (cycleCounter == cycleDelay) {                                           //  A section of code executed every (cycleDelay)th cycle, 
    ...
    if (voltageReading > 5.00 and voltageReading < 7.45) {     //  want it to check (I use Channel T for this)
      digitalWrite(buzzerPin, HIGH);
    }
    else if (voltageReading > 10 and voltageReading < 11.4) {
      digitalWrite(buzzerPin, HIGH);
    }
    else {
